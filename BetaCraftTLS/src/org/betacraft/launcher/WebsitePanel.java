...
public class WebsitePanel extends JPanel {
	...
	/*Whoever put the actual server list and handlers in the source code are insane, 
    this can be a big issue and cause bot problems on offline servers, among many other potential problems.
    Please reconsider how this is done, -Nukley*/
				...
				new Thread() {
					public void run() {
						try {
							...
							textPane.setText(bob.toString().replace("https://", "http://"));
							s.close();
							con.disconnect();
						} catch (UnknownHostException ex) {
							textPane.setContentType("text/html");
